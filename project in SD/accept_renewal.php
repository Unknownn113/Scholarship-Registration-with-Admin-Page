<?php
// filepath: c:\xampp\htdocs\project in SD\accept_renewal.php
session_start();
...
if (isset($_GET['student_number'])) {
    ...
    if ($updateStatusStmt->execute()) {
        echo "<script>
                alert('Renewal request accepted. The student\'s status is now Active.');
                window.location.href = 'index-admin.php'; // Redirect back to the admin dashboard
              </script>";
    } else {
        echo "<script>
                alert('An error occurred while accepting the renewal request. Please try again.');
                window.location.href = 'index-admin.php'; // Redirect back to the admin dashboard
              </script>";
    }
