...
// Verify the admin role in the database
$studentNumber = $_SESSION['admin_student_number'];
...
if ($result->num_rows === 0) {
    // If the admin is not found in the database, log them out
    session_destroy();
    header("Location: login.php");
    exit();
}
...
$studentsQuery = "SELECT student_number, name, email FROM basic_info";
...
<head>
    ...
    <script>
        function toggleSection(sectionIdToShow) {
            const sections = ["students-section", "requirements-section", "renewal-section", "accounts-section"];
...
<div class="sidebar">
    ...
    <a href="javascript:void(0);" onclick="toggleSection('accounts-section')">Student Accounts</a>
    ...
    <table>
        ...
        <tbody>
            ...
            if ($requirementsResult->num_rows > 0) {
                ...
                echo "<tr><td colspan='4'>No submitted requirements found.</td></tr>";
            }
            ?>
        </tbody>
    </table>
            ...
            if ($renewalFormsResult->num_rows > 0) {
                while ($row = $renewalFormsResult->fetch_assoc()) {
                    ...
                    echo "<tr>
                        ...
                        <td>{$row['status']}</td>
                        <td>
                            <a href='accept_renewal.php?student_number={$row['student_number']}' onclick='return confirm(\"Accept this request?\")'>Accept</a> |
                            <a href='deny_renewal.php?student_number={$row['student_number']}' onclick='return confirm(\"Deny this request?\")'>Deny</a>
                        </td>
                    </tr>";
                }
            } else {
            ...
            if ($studentsResult->num_rows > 0) {
                while ($row = $studentsResult->fetch_assoc()) {
                    echo "<tr>
                        ...
                        <td>{$row['email']}</td>
                        <td>
                            <a href='delete_account.php?student_number={$row['student_number']}' onclick='return confirm(\"Are you sure you want to delete this account?\")'>Delete</a>
                        </td>
                    </tr>";
                }
            } else {
                echo "<tr><td colspan='4'>No student accounts found.</td></tr>";
            }
            ?>
        </tbody>
    </table>
